...
export default class FuturesService {
  ...
  public internalFuturesMarkets: Partial<Record<NetworkId, { [marketAddress: string]: PerpsV2MarketInternal }>> = {}
  ...
  public async getMarkets(networkOverride?: NetworkOverrideOptions) {
    ...
    if (this.sdk.context.isMainnet) {
      ...
      const firstResponses = await this.sdk.context.multicallProvider.all(parametersCalls.slice(0, 20))
      const secondResponses = await this.sdk.context.multicallProvider.all(
        parametersCalls.slice(20, parametersCalls.length)
      )
      marketParameters = [...firstResponses, ...secondResponses] as IPerpsV2MarketSettings.ParametersStructOutput[]
    }
    ...
    const futuresMarkets = filteredMarkets.map(
      (
        ...
        openInterest: {
          shortPct: wei(marketSize).eq(0) ? 0 : wei(marketSize).sub(marketSkew).div('2').div(marketSize).toNumber(),
          longPct: wei(marketSize).eq(0) ? 0 : wei(marketSize).add(marketSkew).div('2').div(marketSize).toNumber(),
  ...
  public async getProxiedMarkets(networkOverride?: NetworkOverrideOptions) {
    ...
    const { PerpsV2MarketData, PerpsV2MarketSettings } = this.sdk.context.multicallContracts
  ...
  public async getAverageFundingRates(markets: FuturesMarket[], prices: PricesMap, period: Period) {
    const fundingRateInputs: FundingRateInput[] = markets.map(({ asset, market, currentFundingRate }) => {
  ...
  public async getIsolatedMarginTransfers(walletAddress?: string | null): Promise<MarginTransfer[]> {
    const address = walletAddress ?? this.sdk.context.walletAddress
  ...
  public async getConditionalOrders(account: string) {
    ...
    const contractOrders = (await this.sdk.context.multicallProvider.all(orderCalls)) as ConditionalOrderResult[]
  ...
  public async getIsolatedTradePreview(
    ...
    const market = PerpsV2Market__factory.connect(marketAddress, this.sdk.context.provider)
    const details = await market.postTradeDetails(inputs.sizeDelta.toBN(), inputs.price.toBN(), orderType, user)

    const skewAdjustedPrice = await this.getSkewAdjustedPrice(wei(details.price), marketAddress, marketKey)
  ...
  public async getFillPrice(marketAddress: string, sizeDelta: Wei) {
    const market = PerpsV2Market__factory.connect(marketAddress, this.sdk.context.provider)
  ...
  public async getCrossMarginTradePreview(
    ...
    const skewAdjustedPrice = await this.getSkewAdjustedPrice(tradeParams.orderPrice, marketAddress, marketKey)
  ...
  public async getAllTrades(walletAddress: string, accountType: FuturesAccountType, pageLength: number = 16) {
    const response = await queryTrades(this.sdk, {
      walletAddress,
      accountType,
      pageLength
  ...
  public async getIdleMarginInMarketsCached(accountOrEoa: string, markets: Partial<FuturesMarket>[]) {
    ...
    const positionsWithIdleMargin = positions.filter((p) => !p.position?.size.abs().gt(0) && p.remainingMargin.gt(0))
  ...
  public async withdrawCrossMarginAccount(crossMarginAddress: string, amount: Wei) {
    ...
    return await this.sdk.transactions.getContractTxn(crossMarginAccountContract, 'execute', [commands, inputs])
  }

  public async modifySmartMarginMarketMargin(crossMarginAddress: string, marketAddress: string, marginDelta: Wei) {
    const crossMarginAccountContract = SmartMarginAccount__factory.connect(
      crossMarginAddress,
      this.sdk.context.provider
    )
    ...
    if (marginDelta.gt(0)) {
      ...
      if (marginDelta.gt(freeMargin)) {
        ...
        const depositAmount = marginDelta.gt(totalFreeMargin) ? marginDelta.sub(totalFreeMargin).abs() : wei(0)
  ...
  public async withdrawIsolatedMargin(marketAddress: string, amount: Wei) {
    const market = PerpsV2Market__factory.connect(marketAddress, this.sdk.context.provider)
    const txn = await this.sdk.transactions.getContractTxn(market, 'transferMargin', [amount.neg().toBN()])
    return txn
  }
  ...
  public async submitIsolatedMarginOrder(marketAddress: string, sizeDelta: Wei, priceImpactDelta: Wei) {
    const market = PerpsV2Market__factory.connect(marketAddress, this.sdk.context.provider)

    return await this.sdk.transactions.getContractTxn(market, 'submitOffchainDelayedOrderWithTracking', [
      sizeDelta.toBN(),
  ...
  public async cancelDelayedOrder(marketAddress: string, account: string, isOffchain: boolean) {
    ...
    return isOffchain
      ? await this.sdk.transactions.getContractTxn(market, 'cancelOffchainDelayedOrder', [account])
      : await this.sdk.transactions.getContractTxn(market, 'cancelDelayedOrder', [account])
  }
  ...
  public async executeDelayedOffchainOrder(marketKey: FuturesMarketKey, marketAddress: string, account: string) {
  ...
  public async submitCrossMarginOrder(
    ...
    if (order.sizeDelta.abs().gt(0)) {
      if (!order.conditionalOrderInputs) {
        ...
        commands.push(AccountExecuteFunctions.PERPS_V2_SUBMIT_OFFCHAIN_DELAYED_ORDER)
        inputs.push(encodeSubmitOffchainOrderParams(market.address, order.sizeDelta, order.desiredFillPrice))
      } else {
  ...
  public async updateStopLossAndTakeProfit(
    ...
    if (params.takeProfit || params.stopLoss) {
      ...
      const existingStopLosses = existingOrdersForMarket.filter(
        (o) => o.size.abs().eq(SL_TP_MAX_SIZE) && o.reduceOnly && o.orderType === ConditionalOrderTypeEnum.STOP
      )
      const existingTakeProfits = existingOrdersForMarket.filter(
        (o) => o.size.abs().eq(SL_TP_MAX_SIZE) && o.reduceOnly && o.orderType === ConditionalOrderTypeEnum.LIMIT
      )

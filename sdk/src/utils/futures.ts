...
export const calculateVolumes = (futuresHourlyStats: FuturesAggregateStatResult[]): FuturesVolumes => {
  const volumes: FuturesVolumes = futuresHourlyStats.reduce((acc: FuturesVolumes, { marketKey, volume, trades }) => {
  ...
  return {
    ...
    position: wei(size).eq(ZERO_WEI)
      ...
      : {
          ...
          initialLeverage: initialMargin.gt(0) ? wei(size).mul(wei(lastPrice)).div(initialMargin).abs() : wei(0),
          pnl,
          pnlPct,
          marginRatio: wei(notionalValue).eq(ZERO_WEI) ? ZERO_WEI : wei(remainingMargin).div(wei(notionalValue).abs()),
          leverage: wei(remainingMargin).eq(ZERO_WEI) ? ZERO_WEI : wei(notionalValue).div(wei(remainingMargin)).abs()
        }
  }
}

export const mapFuturesPositions = (futuresPositions: FuturesPositionResult[]): FuturesPositionHistory[] => {
...
export const formatPotentialTrade = (
  ...
  const priceImpact = skewAdjustedPrice.gt(wei(0))
    ? wei(price).sub(skewAdjustedPrice).div(skewAdjustedPrice).abs()
    : wei(0)
...
export const mapMarginTransfers = (marginTransfers: FuturesMarginTransferResult[]): MarginTransfer[] => {
  return marginTransfers.map(({ timestamp, account, market, size, asset, txHash }): MarginTransfer => {
...
export const mapSmartMarginTransfers = (marginTransfers: CrossMarginAccountTransferResult[]): MarginTransfer[] => {
...
export const calculateFeesForAccount = (feesForAccount: Pick<FuturesTradeResult, 'feesPaid' | 'keeperFeesPaid'>[]) => {
...
export const encodeSubmitOffchainOrderParams = (marketAddress: string, sizeDelta: Wei, desiredFillPrice: Wei) => {
  return defaultAbiCoder.encode(
...
export const formatOrderDisplayType = (orderType: ConditionalOrderTypeEnum, reduceOnly: boolean) => {
  if (reduceOnly) {
...
export const calculateDesiredFillPrice = (sizeDelta: Wei, marketPrice: Wei, priceImpactPercent: Wei) => {
...
export const AssetDisplayByAsset: Record<FuturesMarketAsset, string> = {
  ...
  [FuturesMarketAsset.TIA]: 'Tia',
  [FuturesMarketAsset.IMX]: 'ImmutableX',
  [FuturesMarketAsset.MEME]: 'Meme',
  [FuturesMarketAsset.FET]: 'Fetch.ai',
  [FuturesMarketAsset.GRT]: 'The Graph',
  [FuturesMarketAsset.PYTH]: 'Pyth',
  [FuturesMarketAsset.ANKR]: 'Ankr',
